...
async function updateQuoteDDBObject() {
    const quoteTableName = proces.env.API_INSPIRATIONALQUOTES_QUOTEAPPDATATABLE_NAME;
    ...
    try{
        ...
        console.log('error updating quote ovject in DynamoDB', error)
    }
}
    ...
    async function getRandomQuote(apiURLInput) {
        ...
        const svgImage = `
          <svg width="${width}" height="${height}">
              ...
              <text x="${width / 2}" y="${
          height - 10
        }" class="footerStyles">❤︎❤︎❤︎❤︎❤︎</text>
          </svg>
        `;
        ...
        const image = await sharp(selectedBackgroundImage)
          ...
          //Function: UPdate DynamoDB objeto tabla
